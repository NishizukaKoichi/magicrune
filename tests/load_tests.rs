...
fn run_concurrent_load_test(num_threads: usize, requests_per_thread: usize) {
    ...
    println!("Starting load test: {} threads, {} requests each", num_threads, requests_per_thread);
    ...
    println!("Successful: {} ({:.1}%)", success_total, (success_total as f64 / total_requests as f64) * 100.0);
    println!("Failed: {} ({:.1}%)", failure_total, (failure_total as f64 / total_requests as f64) * 100.0);
    ...
    if total_requests >= 100 { // Only enforce for large tests
        assert!(success_total as f64 / total_requests as f64 >= 0.95, "Success rate should be >= 95%");
        assert!(throughput >= 50.0, "Throughput should be >= 50 req/s");
    } else {
        // For small tests, just ensure most succeed
        assert!(success_total as f64 / total_requests as f64 >= 0.8, "Success rate should be >= 80%");
    }
}
...
fn load_test_sustained_throughput() {
    ...
    println!("Starting sustained throughput test: {} req/s for {}s", target_rate, duration_secs);
    ...
    println!("Total requests: {}", total_requests);
    println!("Successful: {} ({:.1}%)", total_success, (total_success as f64 / total_requests as f64) * 100.0);
    ...
    assert!(actual_rate >= target_rate * 0.9, "Should maintain at least 90% of target rate");
}
...
fn load_test_stress_memory() {
    ...
    for i in 0..num_requests {
        ...
        println!("Request {} ({}KB): {} in {:?}", i, file_size_kb, 
    ...
    println!("Successful: {} ({:.1}%)", successful, (successful as f64 / num_requests as f64) * 100.0);
    
    // Should handle reasonable file sizes
    assert!(successful >= num_requests / 2, "Should handle at least 50% of requests under memory pressure");
}
